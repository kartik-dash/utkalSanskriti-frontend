...
import { setSelectedWest } from '../../redux/slices/categorySlice';
const WestCard = () => {   
   const dispatch = useDispatch();
   const navigate = useNavigate();
       const handleWestClick = (product) => {
              dispatch(setSelectedWest(product)); // Set the selected Chari Khetra in Redux
              navigate(`/west/${product.id}`); // Navigate to Chari Khetra Details Page
       };
   ...
   <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-[30px]">
        {categories.map((category) =>
          category.products
            .filter((product) => product.west === "yes")
            .map((product) => (
              <div
                key={product.id}
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <button
                    onClick={() => handleWestClick(product)}
