...
import { selectSelectedSouth } from '../../redux/slices/categorySlice';
...
const SouthDetails = () => {
  ...
  const south = useSelector(selectSelectedSouth);
  ...
  if (!south) return <p className="text-center text-gray-600">Please select a temple.</p>;

  // State for selected image
  const [selectedImage, setSelectedImage] = useState(south?.image_arr?.[0] || null);
    ...
    <div className="px-4 py-2 mt-20 max-w-7xl mx-auto animate-fadeIn bg-gray-600 rounded-lg shadow-xl">
      ...
      <div className="flex flex-col lg:flex-row gap-8 p-6 bg-white rounded-lg shadow-md">
          ...
          <div className="flex mt-6 space-x-4 overflow-x-auto py-2">
            {south.image_arr?.map((img, index) => (
              ...
              >
                <img
                  src={img}
                  alt={`${south.name} thumbnail ${index}`}
        ...
        <div className="w-full lg:w-1/2">
          <h2 className="text-4xl font-bold text-gray-800">{south.name}</h2>
          <p className="text-xl text-gray-600 mt-2">{south.location}</p>
          {/* <p className="text-lg text-gray-700 mt-4">{south.description}</p> */}
          <p className="text-lg text-gray-700 mt-4 whitespace-pre-line">{south.details}</p>
      ...
      {activeSection === 'knowMore' && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow-inner">
          ...
          {south.rituals_img?.map((img, index) => (
              ...
              <div className="flex flex-col lg:flex-row gap-4">
                ...
                <div className="w-full lg:w-1/2">
                  <p className="text-2xl font-bold text-gray-700">
                    {south.rituals_name?.[index] || "Ritual name not available"}
                  </p>
                  <p className="text-md sm:text-lg text-gray-700 mt-2">
                    {south.about_rituals?.[index] || "Description not available"}
                  </p>
                </div>
              </div>

              {/* Optional separator */}
              {index < south.rituals_img.length - 1 && <hr className="my-6 border-gray-200" />}
            </div>
          ))}

          {/* Show message if no rituals exist */}
          {!south.rituals_img?.length && (
            <p className="text-gray-500 mt-4">No rituals information available.</p>
          )}
        </div>
      )}
      {/* Calendar Section */}
      {activeSection === 'calendar' && south.calender && <CalendarComponent calendar={south.calender} />}

      {/* Festival Section */}
      {activeSection === 'festivals' && south.festivals && <TempleFestivals festivals={south.festivals} />}
      {activeSection === 'history' && south.history && <TempleHistory history={south.history} />}

      {/* Map Component */}
      <MapComponent product={south} userLocation={userLocation} />
    </div>
  );
};
