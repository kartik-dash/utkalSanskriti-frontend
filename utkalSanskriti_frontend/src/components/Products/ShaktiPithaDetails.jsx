...
import { selectSelectedShaktiPitha } from '../../redux/slices/categorySlice';
...
const ShaktiPithaDetails = () => {
  const navigate=useNavigate();
  // Fetching ChariKhetra object from Redux store
  const shaktiPitha = useSelector(selectSelectedShaktiPitha);

  // If chariKhetra is undefined or null, show a fallback message
  if (!shaktiPitha) return <p className="text-center text-gray-600">Please select a Shakti Pitha.</p>;

  // State for selected image
  const [selectedImage, setSelectedImage] = useState(shaktiPitha?.image_arr?.[0] || null);
    ...
    <div className="px-4 py-20 pt-10 sm:pt-32 max-w-7xl mx-auto animate-fadeIn bg-gray-600 rounded-lg shadow-xl">
      ...
      <div className="flex flex-col lg:flex-row gap-8 p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="flex-shrink-0 w-full lg:w-1/2">
          ...
          <div className="flex mt-6 space-x-4 overflow-x-auto py-2">
            {shaktiPitha.image_arr?.map((img, index) => (
              <div
                key={index}
              ...
              >
                <img
                  src={img}
                  alt={`${shaktiPitha.name} thumbnail ${index}`}
        ...
        <div className="w-full lg:w-1/2">
          <h2 className="text-xl sm:text-3xl font-bold text-gray-800">{shaktiPitha.name}</h2>
          <p className="text-xl sm:text-2xl text-gray-600 mt-2">{shaktiPitha.location}</p>
          {/* <p className="text-md sm:text-xl text-gray-700 mt-4">{shaktiPitha.description}</p> */}
          <p className="text-md sm:text-xl text-gray-700 mt-4 whitespace-pre-line text-justify">{shaktiPitha.details}</p>
      ...
      {activeSection === "knowMore" && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow-inner">
          ...
          {shaktiPitha.rituals_img?.map((img, index) => (
              ...
              <div className="flex flex-col lg:flex-row gap-4">
                ...
                <div className="w-full lg:w-1/2">
                  <p className="text-2xl  text-bold text-gray-700">
                    {shaktiPitha.rituals_name?.[index] || 'Description not available'}
                  </p>
                  <p className="text-md sm:text-lg text-gray-700 mt-2">
                    {shaktiPitha.about_rituals?.[index] || 'Description not available'}
                  </p>
                </div>
              </div>

              {/* Optional separator */}
              {index < shaktiPitha.rituals_img.length - 1 && (
          ...
          {!shaktiPitha.rituals_img?.length && (
            <p className="text-gray-500 mt-4">No rituals information available.</p>
          )}
        </div>
      )}
      {activeSection === 'calendar' && <CalendarComponent calendar={shaktiPitha.calender} />}
      {activeSection === 'festivals' && <TempleFestivals festivals={shaktiPitha.festivals} />}
      {activeSection === 'history' && <TempleHistory history={shaktiPitha.history} />}

      {/* Map Section */}
      <div className="mt-10">
        <h3 className="text-2xl font-semibold text-gray-800 mb-4">Location Map</h3>
        <MapComponent product={shaktiPitha} userLocation={userLocation} />
      </div>
    </div>
  )
}

export default ShaktiPithaDetails