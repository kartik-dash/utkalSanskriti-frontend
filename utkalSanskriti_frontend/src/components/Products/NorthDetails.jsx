...
import { selectSelectedNorth } from "../../redux/slices/categorySlice";
...
const NorthDetails = () => {
  ...
  const north = useSelector(selectSelectedNorth);
  ...
  if (!north) return <p className="text-center text-gray-600">Please select a temple.</p>;

  // State for selected image
  const [selectedImage, setSelectedImage] = useState(north?.image_arr?.[0] || null);
    ...
    <div className="px-4 py-2 mt-20 max-w-7xl mx-auto animate-fadeIn bg-gray-600 rounded-lg shadow-xl">
      ...
      <div className="flex flex-col lg:flex-row gap-8 p-6 bg-white rounded-lg shadow-md">
          ...
          <div className="flex mt-6 space-x-4 overflow-x-auto py-2">
            {north.image_arr?.map((img, index) => (
              ...
              >
                <img
                  src={img}
                  alt={`${north.name} thumbnail ${index}`}
        ...
        <div className="w-full lg:w-1/2">
          <h2 className="text-4xl font-bold text-gray-800">{north.name}</h2>
          <p className="text-xl text-gray-600 mt-2">{north.location}</p>
          {/* <p className="text-lg text-gray-700 mt-4">{north.description}</p> */}
          <p className="text-lg text-gray-700 mt-4 whitespace-pre-line">{north.details}</p>
      ...
      {activeSection === 'knowMore' && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg shadow-inner">
          ...
          {north.rituals_img?.map((img, index) => (
              ...
              <div className="flex flex-col lg:flex-row gap-4">
                ...
                <div className="w-full lg:w-1/2">
                  <p className="text-2xl font-bold text-gray-700">
                    {north.rituals_name?.[index] || "Ritual name not available"}
                  </p>
                  <p className="text-md sm:text-lg text-gray-700 mt-2">
                    {north.about_rituals?.[index] || "Description not available"}
                  </p>
                </div>
              </div>

              {/* Optional separator */}
              {index < north.rituals_img.length - 1 && <hr className="my-6 border-gray-200" />}
            </div>
          ))}

          {/* Show message if no rituals exist */}
          {!north.rituals_img?.length && (
            <p className="text-gray-500 mt-4">No rituals information available.</p>
          )}
        </div>
      )}
      {/* Calendar Section */}
      {activeSection === 'calendar' && north.calender && <CalendarComponent calendar={north.calender} />}

      {/* Festival Section */}
      {activeSection === 'festivals' && north.festivals && <TempleFestivals festivals={north.festivals} />}
      {activeSection === 'history' && north.history && <TempleHistory history={north.history} />}

      {/* Map Component */}
      <MapComponent product={north} userLocation={userLocation} />
    </div>
  );
};
