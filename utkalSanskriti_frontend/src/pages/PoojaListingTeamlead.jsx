...
const PoojaListing = () => {
  ...
  const poojalistPerPage = 5;

// Pagination logic
const indexOfLastPoojalist = currentPage * poojalistPerPage;
const indexOfFirstPoojalist = indexOfLastPoojalist - poojalistPerPage;
const currentPoojalist = bookings.slice(indexOfFirstPoojalist, indexOfLastPoojalist);
const totalPages = Math.ceil(bookings.length / poojalistPerPage);
    ...
    const fetchBookings = async () => {
      try {
        const response = await fetch(
          `${API_URL}/api/bookings/teamLeader/${userId}`
        );
        if (!response.ok) {
