...
const TempleAdminViewmore = () => {
  ...
  // Function to block a guide
  const handleBlock = async (guideId) => {
    try {
      const token = sessionStorage.getItem("token");
      const response = await fetch(`${API_URL}/api/guide/block/${guideId}`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to block guide");
      }
      const data = await response.json();
      alert(data.message);
      // Optionally re-fetch the users list after a successful block
      dispatch(fetchAdminUsers({ userId, role }));
    } catch (err) {
      alert(err.message);
    }
          ...
          <table className="w-full border-collapse border border-gray-300 whitespace-nowrap">
            <thead>
              <tr className="text-sm sm:text-base bg-gray-200">
                {["Full Name", "Email", "Contact Number", "Status", "Registration Date", "Actions"].map((heading) => (
              ...
              {currentUsers.length > 0 ? (
                currentUsers.map((user) => (
                  <tr key={user.userId} className="text-center">
                    ...
                    <td className="text-xs sm:text-base border p-2">{user.status || "N/A"}</td>
                    ...
                    <td className="text-xs sm:text-base border p-2">
                      {user.status === "BLOCKED" ? (
                        ...
                        <button
                          ...
                          className="bg-red-500 text-xs sm:text-base hover:bg-red-600 text-white px-3 py-1 rounded"
                        >
                          Block
                        </button>
                      )}
...
export default TempleAdminViewmore;
