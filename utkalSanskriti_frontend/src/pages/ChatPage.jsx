...
const ChatPage = () => {
  ...
  const roles = [
    { id: 1, displayName: 'ADMIN', apiRole: 'ADMIN' },
      ...
      <div className="w-full md:w-1/4 bg-white p-4 border-b md:border-b-0 md:border-r border-gray-300 overflow-y-auto">
        ...
        <ul className="space-y-4">
          {roles.map((role) => (
            <li key={role.id}>
              <div
                className="flex items-center cursor-pointer p-2 sm:p-3 hover:bg-[#25D366] hover:text-white rounded-xl transition-all"
                onClick={() => handleRoleSelect(role)}
              >
                <span className="text-sm md:text-lg font-medium">{role.displayName}</span>
              </div>
